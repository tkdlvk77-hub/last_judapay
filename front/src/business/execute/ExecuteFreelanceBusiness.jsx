...
export default function ExecuteFreelanceBusiness() {
  ...
  // 마케팅비 분기 (외주비 화면 재사용)
  const menuParam = searchParams.get('menu') || 'freelance'
  const isMarketing = menuParam === 'marketing'
  const txType = isMarketing ? 'marketing' : 'freelance'
  const txLabel = isMarketing ? '마케팅비' : '외주비'
  const contractTitle = isMarketing ? '마케팅 용역 계약서' : '외주 용역 계약서'
  const fileBackPath = `/execute/business/select-vendor?menu=${menuParam}`

  useEffect(() => {
    if (!recipient) {
      navigate(fileBackPath, { replace:true })
    }
  }, [recipient, navigate, fileBackPath])
  ...
  // 사업자는 정상/폐업 분기. 개인은 가입자(인증) / 휴대폰만 입력(외부링크 미인증) 분기
  const isCashable = recipient.isBusiness ? !recipient.riskAccepted : recipient.freelancer === true
  const fundLabel = isCashable ? t('execFreelance.fund.cashable') : t('execFreelance.fund.permission')
  const fundBadgeTone = isCashable ? 'cashable' : 'permission'

  // step1 서브타이틀 — 수신자 유형 (기업 발주 컨텍스트)
  const subtitleKey = (() => {
    if (recipient.isBusiness) {
      return recipient.riskAccepted
        ? 'execFreelance.sub.bizClosed.business'
        : 'execFreelance.sub.bizNormal.business'
    }
    return recipient.freelancer
      ? 'execFreelance.sub.freelancer.business'
      : 'execFreelance.sub.unverified.business'
  })()
  const step1Sub = fill(t(subtitleKey), { name: recipient.name })
  ...
  // 원천세 3.3% 계산 (방식 ①: 외주비에서 차감 후 입금)
  // - 인증자(사업자/프리랜서)만 적용. 미인증자는 권한 자금 보관 → 외부링크 인증 후 처리
  const WHT_RATE = 0.033
  const whtAmount = isCashable ? Math.floor(amtNum * WHT_RATE) : 0
  const netAmount = amtNum - whtAmount
  const whtFmt = whtAmount.toLocaleString('ko-KR')
  const netFmt = netAmount.toLocaleString('ko-KR')
  ...
  const inspectionTimingLabel = INSPECTION_TIMINGS.find(it => it.id === inspectionTiming)?.label || ''

  const changeRecipient = () => {
    if (recipient.isBusiness) {
      navigate(`/execute/business/select-vendor?menu=${menuParam}`)
    } else {
      // 기업이 개인 받는 사람 다시 선택 → SelectRecipientBusiness
      navigate(`/execute/business/select-recipient?menu=${menuParam}`)
    }
  }
    ...
    <PhoneShell>
      <div style={{ flex:1, overflowY:'auto' }}>
        <DarkHeader
          ...
          step={1} totalSteps={4}
          bigTitle={t('execFreelance.step1.title')}
          sub={step1Sub}
          onBack={goBack}
          headerGrad={theme.headerGrad}
          exitTo="/home-business"
        ...
        <div style={{ padding:'18px 16px 24px' }}>
          ...
          <div style={{
                ...
                {recipient.isBusiness ? (
                  <span style={{
                    ...
                    {recipient.riskAccepted
                      ? t('execFreelance.recipient.bizClosed')
                      : t('execFreelance.recipient.bizNormal')}
                  </span>
                ) : (
                  <>
                    {recipient.verified && (
                    ...
                    {recipient.freelancer && (
                      <span style={{
                        ...
                        {t('execFreelance.recipient.freelancer')}
                      </span>
                    )}
                  </>
                )}
            ...
            <button onClick={changeRecipient}
              ...
              {t('execFreelance.recipient.change')}
            </button>
          </div>

          {/* 미인증 수신자 안내 — 외부링크 인증 후 출금 전환 */}
          {!isCashable && (
            <div style={{
              background:'#FFFBEB',
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontSize:'12px', fontWeight:700, color:'#854F0B', marginBottom:'4px' }}>
                  {t('execFreelance.unverified.title')}
                </div>
                <div style={{ fontSize:'11px', color:'#854F0B', lineHeight:1.6 }}>
                  {fill(t('execFreelance.unverified.body'), { name: recipient.name })}
                </div>
              </div>
            </div>
          )}
  ...
  if (step === 4) return (
    <PhoneShell>
      <div style={{ flex:1, overflowY:'auto' }}>
        <DarkHeader
          smallTitle={t('execFreelance.contract.smallTitle')}
          badge={payMode === 'split' ? t('execFreelance.contract.modeSplit') : t('execFreelance.contract.modeSingle')}
          step={4} totalSteps={4}
          bigTitle={t('execFreelance.contract.title')}
          sub={t('execFreelance.contract.sub')}
          onBack={goBack}
          headerGrad={theme.headerGrad}
          exitTo="/home-business"
        />
  ...
  // ───────────── 확인 (집행 내용 확인) ─────────────
  if (step === 'confirm') {
    const modeLabel = payMode === 'single'
      ? t('execFreelance.row.paySingle')
      : t('execFreelance.row.paySplit')
    const inspLabel = INSPECTION_TIMINGS.find(it => it.id === inspectionTiming)?.label || ''
    const walletLabel = selectedWallet?.label || t('execFreelance.source.business')

    // 원천세/실수령액 행 (인증자만)
    const whtRows = isCashable && amtNum > 0 ? [
      {
        label: t('execFreelance.wht.label'),
        value: `-${whtFmt}원`,
        sub: t('execFreelance.wht.autoDeduct'),
      },
      {
        label: t('execFreelance.wht.netLabel'),
        value: `${netFmt}원`,
    ...
    ] : (!isCashable && amtNum > 0 ? [
      {
        label: t('execFreelance.wht.label'),
        value: '—',
        sub: t('execFreelance.wht.notApplicable'),
      },
    ] : [])

    return (
      <ConfirmStep
        smallTitle={t('execFreelance.confirm.smallTitle')}
        bigAmount={`${amtFmt}원`}
        sub={fill(t('execFreelance.confirm.subBusiness'), { name: recipient.name, fundLabel })}
        onBack={goBack}
        headerGrad={theme.headerGrad}
        rows={[
          {
            label: t('execFreelance.row.recipient'),
            value: recipient.name,
            sub: recipient.isBusiness
              ? fill(t('execFreelance.row.recipientBizSub'), { bizNumber: recipient.bizNumber })
              : t('execFreelance.row.recipientKycSub'),
            editAction: () => navigate(recipient.isBusiness
              ? `/execute/business/select-vendor?menu=${menuParam}`
              : `/execute/business/select-recipient?menu=${menuParam}`),
          },
          {
            label: t('execFreelance.source.business'),
            value: walletLabel,
            sub: fill(t('execFreelance.row.balanceLabel'), { balance: walletBalance.toLocaleString() }),
            editAction: () => setStep(3),
          },
          ...whtRows,
          {
            label: t('execFreelance.row.payMode'),
            value: modeLabel,
            editAction: () => setStep(2),
          },
          {
            label: t('execFreelance.row.inspection'),
            value: inspLabel,
            editAction: () => setStep(3),
          },
          ...(payMode === 'split' ? [{
            label: t('execFreelance.row.splitRatio'),
            value: splitRatios.map(r => `${r.label} ${r.pct}%`).join(' / '),
            editAction: () => setStep(3),
          }] : []),
          {
            label: t('execFreelance.row.contract'),
            value: t('execFreelance.row.contractAuto'),
            sub: t('execFreelance.row.contractSign'),
          },
        ]}
        autoActions={[
          fill(t('execFreelance.auto.smsSent'), { name: recipient.name }),
          isCashable
            ? t('execFreelance.auto.signedCashable')
            : t('execFreelance.auto.signedPermission'),
          inspectionTiming === 'after_inspection'
            ? fill(t('execFreelance.auto.afterInsp'), { name: recipient.name })
            : t('execFreelance.auto.afterDeadline'),
          ...(isCashable ? [t('execFreelance.tax.autoInvoice')] : []),
          t('execFreelance.auto.archive'),
        ]}
        footerNote={
          <>
            {fill(t('execFreelance.footer.afterExec'), {
              wallet: walletLabel,
              before: walletBalance.toLocaleString(),
              after: (walletBalance - amtNum).toLocaleString(),
            })}
          </>
        }
        primaryLabel={t('execFreelance.btn.execute')}
  ...
  // 거래 store에 push (PinStep 완료 시 호출)
  const pushToStore = () => {
    // 풍부 milestones 생성
    let milestones
    ...
    if (payMode === 'single') {
      ...
      milestones = splitRatios.map((r, i) => {
        ...
        // 선금: 서명 완료 시 즉시 입금 (조건 없음)
        // 중도금: 작업물 제출 + 검수 (활성 시 conditions 표시)
        // 잔금: 최종 컨펌 + 마감일 도래
        const note = isFirst
    ...
    // 가입자/미인증/사업자 분기 (계약 진행 단계)
    const dealStatus = !isCashable ? 'waiting' : 'signing'
    const statusLabel = !isCashable
      ? '외부링크 인증 대기'
      : '상대방 서명 대기'
    ...
    const timeline = [
      ...
      ...(workEnd ? [{ time: workEnd, label: `${isMarketing ? '마케팅' : '외주'} 마감일`, type: 'pending' }] : []),
    ]
    ...
    // 계약서 파일명 (사유 + 타임스탬프)
    const safeWorkName = (workName || (isMarketing ? '마케팅' : '외주')).replace(/[^\w가-힣]/g, '_')
    const contractFile = `${isMarketing ? '마케팅' : '외주'}_${safeWorkName}.pdf`
    ...
    addTransaction({
      ...
      dealStatus,
      statusLabel,
      myAction: null,                  // 보낸 사람 입장에선 액션 없음 (상대방 서명 대기 중)
    })
  }
  ...
  if (step === 'pin') return (
    <PinStep
      summaryLeft={`${recipient.name} ${txLabel}`}
  ...
  if (step === 'done') return (
    <DoneStep
      tone="waiting"
      title={fill(t('execFreelance.done.titleWaiting'), { name: recipient.name })}
      description={
        <>
          {fill(t('execFreelance.done.descBusiness'), { name: recipient.name })}
        </>
      }
      summary={[
        { label: txLabel, value:`${amtFmt}원`, accent:true },
        ...(isCashable && amtNum > 0 ? [
          { label: t('execFreelance.wht.label'), value: `-${whtFmt}원` },
          { label: t('execFreelance.wht.netLabel'), value: `${netFmt}원`, bold: true },
        ] : []),
        { label: t('execFreelance.row.payMode'),
          value: payMode === 'single' ? t('execFreelance.row.paySingle') : t('execFreelance.row.paySplit') },
        { label: t('execFreelance.row.inspection'),
          value: INSPECTION_TIMINGS.find(it => it.id === inspectionTiming)?.label || '' },
        { label: t('execFreelance.source.business'),
          value: selectedWallet?.label || t('execFreelance.source.business') },
      ]}
      noteYellow={fill(t('execFreelance.done.noteBusiness'), { name: recipient.name })}
      primaryLabel={t('execFreelance.btn.toHome')}
      onPrimary={() => navigate('/home-business')}
      secondaryLabel={fill(t('execFreelance.btn.chat'), { name: recipient.name })}
